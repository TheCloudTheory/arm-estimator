...
{
    public AppConfigurationRetailQuery(WhatIfChange change, CommonResourceIdentifier id, ILogger logger, CurrencyCode currency, WhatIfChange[] changes, TemplateSchema template) : base(change, id, logger, currency, changes, template)
    {
    }
    ...
    public string? GetQueryUrl(string location)
    {
        ...
        var filter = new AppConfigurationQueryFilter(change, this.logger).GetFiltersBasedOnDesiredState(location);
