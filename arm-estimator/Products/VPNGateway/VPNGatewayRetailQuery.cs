...
{
    public VPNGatewayRetailQuery(WhatIfChange change, ResourceIdentifier id, ILogger logger, CurrencyCode currency, WhatIfChange[] changes) : base(change, id, logger, currency, changes)
    {
    }
    ...
    public string? GetQueryUrl(string location)
    {
        ...
        var filter = new VPNGatewayQueryFilter(change, this.logger).GetFiltersBasedOnDesiredState(location);
