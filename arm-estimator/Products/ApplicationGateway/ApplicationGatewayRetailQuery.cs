...
{
    public ApplicationGatewayRetailQuery(WhatIfChange change, ResourceIdentifier id, ILogger logger, CurrencyCode currency, WhatIfChange[] changes) : base(change, id, logger, currency, changes)
    {
    }
    ...
    public string? GetQueryUrl(string location)
    {
        ...
        var filter = new ApplicationGatewayQueryFilter(change, this.logger).GetFiltersBasedOnDesiredState(location);
