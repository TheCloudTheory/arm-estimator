...
internal class WhatIfProcessor
{
                                                                   ...
                                                                   CancellationToken token) where T : BaseRetailQuery, IRetailQuery
    {
        ...
        {
            return this.mockedRetailAPIResponsePaths != null && this.mockedRetailAPIResponsePaths.Length > 0 && this.mockedRetailAPIResponsePaths.All(p => p.Exists);
        }

        if (ShouldTakeMockedData())
        {
            foreach(var mockedRetailAPIResponsePath in this.mockedRetailAPIResponsePaths!)
            {
                this.logger.LogWarning("Using mocked data from {path}.", mockedRetailAPIResponsePath.FullName);
                var mockedData = await File.ReadAllTextAsync(mockedRetailAPIResponsePath.FullName, token);
                var response = JsonSerializer.Deserialize<RetailAPIResponse>(mockedData);

                if (response == null || response.Url != url)
                {
                    this.logger.LogWarning("Mocked data URL {url} doesn't match the URL generated by the application {generatedUrl}.", response?.Url, url);
                    continue;
                }
            ...
            this.logger.LogError("Couldn't find any mocked data for {type}.", typeof(T));
            return null;
        }
