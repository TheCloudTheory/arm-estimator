internal class AppServicePlanSupportedData
{
    public static readonly IReadOnlyDictionary<string, string> SkuToServiceId = new Dictionary<string, string>()
    {
        { "Y1", "DZH319HJX2WX" },
        { "EP1", "DZH319HJX2WX" },
        { "EP2", "DZH319HJX2WX" },
        { "EP3", "DZH319HJX2WX" },
        { "F1", "DZH317GT8G5N" },
        { "D1", "DZH317GT8G5N" },
        { "B1", "DZH317GT8G5N" },
        { "B2", "DZH317GT8G5N" },
        { "B3", "DZH317GT8G5N" },
        { "S1", "DZH317GT8G5N" },
        { "S2", "DZH317GT8G5N" },
        { "S3", "DZH317GT8G5N" },
        { "P1", "DZH317GT8G5N" },
        { "P2", "DZH317GT8G5N" },
        { "P3", "DZH317GT8G5N" },
        { "P1V2", "DZH317GT8G5N" },
        { "P2V2", "DZH317GT8G5N" },
        { "P3V2", "DZH317GT8G5N" },
        { "P0V3", "DZH317GT8G5N" },
        { "P1V3", "DZH317GT8G5N" },
        { "P2V3", "DZH317GT8G5N" },
        { "P3V3", "DZH317GT8G5N" },
        { "WS1", "DZH319T30TWX" },
        { "WS2", "DZH319T30TWX" },
        { "WS3", "DZH319T30TWX" }
    };

    public static readonly IReadOnlyDictionary<string, string> SkuToSkuNameMap = new Dictionary<string, string>()
    {
        { "Y1",
            "Standard"
        },
        { "EP1",
            "Premium"
        },
        { "EP2",
            "Premium"
        },
        { "EP3",
            "Premium"
        },
        { "F1",
            "F1"
        },
        { "D1",
            "Shared"
        },
        { "B1",
            "B1"
        },
        { "B2",
            "B2"
        },
        { "B3",
            "B3"
        },
        { "S1",
            "S1"
        },
        { "S2",
            "S2"
        },
        { "S3",
            "S3"
        },
        { "P1",
            "P1"
        },
        { "P2",
            "P2"
        },
        { "P3",
            "P3"
        },
        { "P4",
            "P4"
        },
        { "P1V2",
            "P1 v2"
        },
        { "P2V2",
            "P2 v2"
        },
        { "P3V2",
            "P3 v2"
        },
        { "P0V3",
            "P0v3"
        },
        { "P1V3",
            "P1 v3"
        },
        { "P2V3",
            "P2 v3"
        },
        { "P3V3",
            "P3 v3"
        },
        { "WS1",
            "Standard"
        },
        { "WS2",
            "Standard"
        },
        { "WS3",
            "Standard"
        }
    };

    public static readonly IReadOnlyDictionary<string, string[]> SkuToProductNameWindowsMap = new Dictionary<string, string[]>()
    {
        {
            "F1", new[]
            {
                "Azure App Service Free Plan"
            }
        },
        {
            "D1", new[]
            {
                "Azure App Service Shared Plan"
            }
        },
        {
            "B1", new[]
            {
                "Azure App Service Basic Plan"
            }
        },
        {
            "B2", new[]
            {
                "Azure App Service Basic Plan"
            }
        },
        {
            "B3", new[]
            {
                "Azure App Service Basic Plan"
            }
        },
        {
            "S1", new[]
            {
                "Azure App Service Standard Plan"
            }
        },
        {
            "S2", new[]
            {
                "Azure App Service Standard Plan"
            }
        },
        {
            "S3", new[]
            {
                "Azure App Service Standard Plan"
            }
        },
        {
            "P1", new[]
            {
                "Azure App Service Premium Plan"
            }
        },
        {
            "P2", new[]
            {
                "Azure App Service Premium Plan"
            }
        },
        {
            "P3", new[]
            {
                "Azure App Service Premium Plan"
            }
        },
        {
            "P1V2", new[]
            {
                "Azure App Service Premium v2 Plan"
            }
        },
        {
            "P2V2", new[]
            {
                "Azure App Service Premium v2 Plan"
            }
        },
        {
            "P3V2", new[]
            {
                "Azure App Service Premium v2 Plan"
            }
        },
        {
            "P0V3", new[]
            {
                "Azure App Service Premium v3 Plan"
            }
        },
        {
            "P1V3", new[]
            {
                "Azure App Service Premium v3 Plan"
            }
        },
        {
            "P2V3", new[]
            {
                "Azure App Service Premium v3 Plan"
            }
        },
        {
            "P3V3", new[]
            {
                "Azure App Service Premium v3 Plan"
            }
        },
        {
            "Y1", new[]
            {
                "Functions"
            }
        },
        {
            "EP1", new[]
            {
                "Premium Functions"
            }
        },
        {
            "EP2", new[]
            {
                "Premium Functions"
            }
        },
        {
            "EP3", new[]
            {
                "Premium Functions"
            }
        }
    };

    public static readonly IReadOnlyDictionary<string, string[]> SkuToProductNameLinuxMap = new Dictionary<string, string[]>()
    {
        {
            "F1", new[]
            {
                "Azure App Service Free Plan - Linux"
            }
        },
        {
            "B1", new[]
            {
                "Azure App Service Basic Plan - Linux"
            }
        },
        {
            "B2", new[]
            {
                "Azure App Service Basic Plan - Linux"
            }
        },
        {
            "B3", new[]
            {
                "Azure App Service Basic Plan - Linux"
            }
        },
        {
            "S1", new[]
            {
                "Azure App Service Standard Plan - Linux"
            }
        },
        {
            "S2", new[]
            {
                "Azure App Service Standard Plan - Linux"
            }
        },
        {
            "S3", new[]
            {
                "Azure App Service Standard Plan - Linux"
            }
        },
        {
            "P1", new[]
            {
                "Azure App Service Premium Plan - Linux"
            }
        },
        {
            "P2", new[]
            {
                "Azure App Service Premium Plan - Linux"
            }
        },
        {
            "P3", new[]
            {
                "Azure App Service Premium Plan - Linux"
            }
        },
        {
            "P1V2", new[]
            {
                "Azure App Service Premium v2 Plan - Linux"
            }
        },
        {
            "P2V2", new[]
            {
                "Azure App Service Premium v2 Plan - Linux"
            }
        },
        {
            "P3V2", new[]
            {
                "Azure App Service Premium v2 Plan - Linux"
            }
        },
        {
            "P0V3", new[]
            {
                "Azure App Service Premium v3 Plan - Linux"
            }
        },
        {
            "P1V3", new[]
            {
                "Azure App Service Premium v3 Plan - Linux"
            }
        },
        {
            "P2V3", new[]
            {
                "Azure App Service Premium v3 Plan - Linux"
            }
        },
        {
            "P3V3", new[]
            {
                "Azure App Service Premium v3 Plan - Linux"
            }
        },
        {
            "Y1", new[]
            {
                "Functions"
            }
        },
        {
            "EP1", new[]
            {
                "Premium Functions"
            }
        },
        {
            "EP2", new[]
            {
                "Premium Functions"
            }
        },
        {
            "EP3", new[]
            {
                "Premium Functions"
            }
        }
    };
}
