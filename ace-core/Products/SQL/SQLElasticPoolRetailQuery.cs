...
internal class SQLElasticPoolRetailQuery : BaseRetailQuery, IRetailQuery
{
    public SQLElasticPoolRetailQuery(WhatIfChange change, CommonResourceIdentifier id, ILogger logger, CurrencyCode currency, WhatIfChange[] changes, TemplateSchema template) 
    ...
    public string? GetQueryUrl(string location)
    {
        ...
        var filter = new SQLElasticPoolQueryFilter(change, this.logger).GetFiltersBasedOnDesiredState(location);
